...
dependencies {
    ...
    val toolsVersion =
        agpVersion.split('.').run { "${23 + first().toInt()}." + drop(1).joinToString(".") }
...
val versionClassTask =
    tasks.register<Copy>("createVersionClass", Copy::configureCreateVersionClassTask)

sourceSets { main { kotlin.srcDir(genFolder) } }
...
tasks.named<Copy>("processTestResources").configure {
    val tokens =
        mapOf(
            ...
            // This string is delimited with semicolons, and each of the separated values itself is
            // a 4-tuple.
            //
            // Example:
            ...
            //       Compile SDK: 33
            "AGP_VERSIONS" to
                SupportedAgp.values().joinToString(separator = ";") { plugin ->
            ...
            // This string is delimited with semicolons, and each of the separated values is a
            // 2-tuple.
            // Example:
            // JUNIT_VERSIONS = 5|5.14.1;6|6.0.1
            "JUNIT_VERSIONS" to
                SupportedJUnit.values().joinToString(separator = ";") { junit ->
                    val fullVersion =
                        when (junit) {
...
project.configurations.apply {
    SupportedAgp.values().forEach { plugin ->
        create(plugin.configurationName) {
            description =
                "Local dependencies used for compiling & running " +
            ...
            // Acknowledging the minimum required Gradle version, request the correct variant for
            // KGP
            // (see
            // https://docs.gradle.org/current/userguide/implementing_gradle_plugins.html#plugin-with-variants)
...
tasks.named("pluginUnderTestMetadata").configure {
    ...
    configurations
        .filter { it.name.startsWith("testAgp") }
        .forEach { configuration ->
            val strippedName = configuration.name.substring(4).lowercase(Locale.ROOT)
            val prunedFile =
                File(defaultDirectory, "pruned-plugin-metadata-$strippedName.properties")
                ...
                prunedFile.writer().use { writer ->
                    ...
                    val classesDirs =
                        layout.buildDirectory
                            ...
                            ?.map { File(it, "main") }
                            ?.filter {
                                it.exists() && it.isDirectory && it.list()?.isEmpty() == false
                            } ?: emptyList()
                    val resourcesDirs =
                        layout.buildDirectory.dir("resources").get().asFile.listFiles()?.filter {
                            it.isDirectory
                        } ?: emptyList()

                    writer.write("implementation-classpath=")
                    writer.write(
                        (classesDirs + resourcesDirs + configuration).joinToString(
                            separator = "\\:"
                        )
                    )
                }
...
listOf("compileKotlin", "sourcesJar", "dokkaGeneratePublicationHtml").forEach { taskName ->
    tasks.named(taskName).configure { dependsOn(versionClassTask) }
}

...
interface AgpJacocoExclusionRuleTests : AgpVariantAwareTests {
    ...
    fun `add exclusion rules`(): List<DynamicTest> {
        ...
        project.junitPlatform.jacocoOptions { it.excludedClasses.add("Second*.class") }
        project.evaluate()
        ...
        return listOf(
            dynamicTest("honor the debug class exclusion rules") {
                ...
                val fileNames =
                    project.tasks
                        .get<AndroidJUnit5JacocoReport>("${JACOCO_TASK_NAME}Debug")
                        .classDirectories!!
                        .asFileTree
                        .files
                        .map { it.name }
            ...
            dynamicTest("honor the release class exclusion rules") {
                ...
                val fileNames =
                    project.tasks
                        .get<AndroidJUnit5JacocoReport>("${JACOCO_TASK_NAME}Release")
                        .classDirectories!!
                        .asFileTree
                        .files
                        .map { it.name }
    ...
    @TestFactory
    fun `replace exclusion rules`() =
        forEachBuildType(
            beforeBuild = { it.applyJacocoPlugin() },
            beforeEvaluate = { project ->
                project.createFakeFiles()
                project.junitPlatform.jacocoOptions { it.excludedClasses.set(emptyList()) }
            },
        ) { project, buildType ->
            val name = "${JACOCO_TASK_NAME}${buildType.capitalized()}"
            val fileNames =
                project.tasks
                    .get<AndroidJUnit5JacocoReport>(name)
                    .classDirectories!!
                    .asFileTree
                    .files
                    .map { it.name }
...
private fun Project.createFakeFiles() {
    // Since the location of intermediate class files changed in different versions of the Android
    // Gradle Plugin,

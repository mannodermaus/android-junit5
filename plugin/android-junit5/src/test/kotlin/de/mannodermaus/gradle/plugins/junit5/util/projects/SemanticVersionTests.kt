...
class SemanticVersionTests {
    ...
    @ParameterizedTest
    fun `parse version into stable and suffix parts correctly`(
        input: String,
        expectedStable: Int,
        expectedSuffix: Int,
    ) {
    ...
    @ParameterizedTest
    fun `compares versions correctly`(
    ...
    fun `throws on incorrect inputs`(version: String, expectedError: String) {
        val exception = assertThrows<IllegalArgumentException> { SemanticVersion(version) }

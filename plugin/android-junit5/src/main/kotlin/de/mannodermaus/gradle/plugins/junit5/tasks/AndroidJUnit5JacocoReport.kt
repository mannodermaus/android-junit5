...
/**
 * Jacoco Test Reporting Task connected to a variant-aware JUnit 5 task. Required to be "open" in
 * order for Groovy's proxy magic to do its thing.
 */
@CacheableTask
public abstract class AndroidJUnit5JacocoReport : JacocoReport() {
    ...
    internal companion object {
        ...
        private fun findOrRegisterDefaultJacocoTask(project: Project): TaskProvider<Task> =
            project.tasks.namedOrNull(JACOCO_TASK_NAME)
                ?: project.tasks.register(JACOCO_TASK_NAME) { it.group = GROUP_REPORTING }
    }
    ...
    private class ConfigAction(
        ...
        fun execute(reportTask: AndroidJUnit5JacocoReport) {
            ...
            taskJacoco.destinationFile?.let { reportTask.executionData.setFrom(it.path) }
            ...
            reportTask.classDirectories.setFrom(
                directoryProviders
                    ...
                    .toFileCollectionExcluding(junit5Jacoco.excludedClasses.get())
            )
        ...
        /**
         * Joins the given collection of Files together, while ignoring the provided patterns in the
         * resulting FileCollection.
         */

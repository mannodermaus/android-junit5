...
/**
 * Default Provider implementation for Java-based test root directories. This will look up the main
 * & test root directories of the variant connected to a given JUnit 5 task.
 */
internal class JavaDirectoryProvider(private val variant: BaseVariant) : DirectoryProvider {
    ...
    private fun sourceFoldersOf(variant: BaseVariant) =
        variant.sourceSets.flatMap { it.javaDirectories }.toSet()

    private fun classFoldersOf(variant: BaseVariant) =
        setOfNotNull(
            variant.javaCompileProvider.map { it.destinationDirectory.asFile }.get().orNull
        )
}

...
class AndroidJUnit5JacocoReport extends JacocoReport {
    ...
    static class ConfigAction implements TaskConfigAction<AndroidJUnit5JacocoReport> {
        ...
        void execute(AndroidJUnit5JacocoReport reportTask) {
            ...
            reportTask.sourceDirectories = project.files(
                    variant.sourceSets.java
                            .srcDirs
                            .collect { it.path }
                            .flatten())

            // Hook in plugin configuration parameters
            def junitExtension = project.extensions.getByName(AndroidJUnitPlatformPlugin.EXTENSION_NAME) as AndroidJUnitPlatformExtension
            def jacocoExtension = junitExtension.extensions.getByName(AndroidJUnitPlatformPlugin.JACOCO_EXTENSION_NAME) as AndroidJUnit5JacocoExtension
            ...
            project.logger.info("$AndroidJUnitPlatformPlugin.LOG_TAG: Assembled Jacoco Code Coverage for JUnit 5 Task '$testTask.name':")
            project.logger.info("$AndroidJUnitPlatformPlugin.LOG_TAG: |__ Execution Data: ${reportTask.executionData.asPath}")
            project.logger.info("$AndroidJUnitPlatformPlugin.LOG_TAG: |__ Source Dirs: ${reportTask.sourceDirectories.asPath}")
            project.logger.info("$AndroidJUnitPlatformPlugin.LOG_TAG: |__ Class Dirs: ${reportTask.classDirectories.asPath}")

...
abstract class AndroidJUnitPlatformSpec extends Specification {
  ...
  def "basic application setup"() {
    ...
    then:
    def variantTasks = [p.tasks.getByName("junitPlatformTestDebug"),
  ...
  def "application with product flavors"() {
    ...
    then:
    def variantTasks = [p.tasks.getByName("junitPlatformTestFreeDebug"),
  ...
  def "custom junit jupiter version"() {
    ...
    try {
      p.evaluate()
      throw new AssertionError(
          "Expected ${ModuleVersionNotFoundException.class.name}, but wasn't thrown")
  ...
  def "jacoco in library module"() {
    ...
    then:
    def variantTasks = [p.tasks.getByName("jacocoTestReportDebug"),
                        p.tasks.getByName("jacocoTestReportRelease")]
  ...
  def "jacoco in app module with flavors"() {
    ...
    then:
    def variantTasks = [p.tasks.getByName("jacocoTestReportFreeDebug"),

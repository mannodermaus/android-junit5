...
class JupiterTestMethodFinderTests {
        ...
        classes.map { (cls, testCount) ->
            dynamicTest("expect $testCount tests for ${cls.simpleName}") {
                ...
                if (testCount == 0) {
                    ...
                    assertWithMessage(
                            "Executed ${result.count()} instead of $testCount tests on class '${cls.simpleName}': '${result.methodNames()}'"
                        )
                        .that(result.count())
                        .isEqualTo(testCount)
                }
    ...
    fun `check that tag filter works`() {
        val result =
            runJUnit5(cls = HasTaggedTest::class.java, filter = TagFilter.excludeTags("slow"))

        assertWithMessage(
                "Executed ${result.count()} instead of 0 tests: '${result.methodNames()}'"
            )
            .that(result.count())
            .isEqualTo(0)
    }
    ...
    private fun testForEachFinder(block: (JupiterTestMethodFinder) -> List<DynamicNode>) =
        allFinders.map { finder ->
            dynamicContainer("using ${finder.javaClass.simpleName}", block(finder))
        }

    @CheckResult
    private fun runJUnit5(cls: Class<*>, filter: Filter<*>? = null): CountingRunListener {

...
class HasRepeatedTest {
    @RepeatedTest(5) fun method(info: RepetitionInfo) {}
}

class HasTestFactory {
    @TestFactory fun method() = listOf(dynamicTest("a") {}, dynamicTest("b") {})
}

class HasTestTemplate {
    @TestTemplate @ExtendWith(ExampleInvocationContextProvider::class) fun method(param: String) {}
    ...
    class ExampleInvocationContextProvider : TestTemplateInvocationContextProvider {
        override fun provideTestTemplateInvocationContexts(
            ...
            listOf("param1", "param2").map(this::context).stream()
        ...
        private fun context(param: String): TestTemplateInvocationContext =
            object : TestTemplateInvocationContext {
                ...
                override fun getAdditionalExtensions() =
                    listOf(
...
class HasParameterizedTest {
    @ParameterizedTest @CsvSource("a", "b") fun method(param: String) {}
}
...
class HasTaggedTest {
    @Tag("slow") @Test fun method() {}
}

abstract class AbstractTestClass {
    @Test open fun abstractTest() {}
}
...
class HasJUnit4Tests {
    @org.junit.Test fun method() {}
}

@RepeatedTest(2) fun topLevelRepeatedTest(unused: RepetitionInfo) {}

@ValueSource(strings = ["a", "b"])
@ParameterizedTest
fun topLevelParameterizedTest(unused: String) {}

@TestTemplate fun topLevelTestTemplate() {}

@TestFactory fun topLevelTestFactory(): Stream<DynamicNode> = Stream.empty()
